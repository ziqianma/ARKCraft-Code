...
public class InventoryTaming implements IInventory, IUpdatePlayerListBox
{
    ...
    {
        if (feedingTimeInitialValue[foodSlot] <= 0)
        {
            return 0;
        }
    ...
    {
        if (feedingTimeRemaining[foodSlot] <= 0)
        {
            return 0;
        }
    ...
    {
        if (stack.getItem() instanceof ItemARKBase)
        {
            return (short) MathHelper.clamp_int(ItemARKBase.getItemTorporTime(stack), 0, Short.MAX_VALUE);
        }
        if (stack.getItem() instanceof ItemARKFood)
        {
            return (short) MathHelper.clamp_int(ItemARKFood.getItemTorporTime(stack), 0, Short.MAX_VALUE);
        }
        return 0;
    }
