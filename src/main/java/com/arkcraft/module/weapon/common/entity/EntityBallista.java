...
public class EntityBallista extends EntityBoat
{
	...
	public EntityBallista(World world, double d, double d1, double d2)
	{
		this(world);
		setPosition(d, d1, d2);
	...
	public boolean attackEntityFrom(DamageSource damagesource, float damage)
	{
		...
		else if (damagesource instanceof EntityDamageSource)
		{
			...
			{
				EntityPlayer player = (EntityPlayer) (((EntityDamageSource) damagesource)
						.getEntity());
	...
	/*
	 * @Override protected void fall(float f) { super.fall(f); int i =
	 * MathHelper.floor_float(f); i *= 2; attackEntityFrom(DamageSource.fall,
	 * i); }
	 */
	@Override
	...
	public void handleReloadTime()
	{
		...
		if (l > 0)
		{
			if (l == 80 || l == 70 || l == 60)
			{
				worldObj.playSoundAtEntity(this, "tile.piston.in", 0.5F,
						1.2F / (rand.nextFloat() * 0.8F + 0.6F));
			}
			else if (l == 40)
			{
	...
	public void fireBallista()
	{
		if (!isLoaded()) { return; }
		// if (!worldObj.isRemote)
		// {
		EntityBallistaBolt entitycannonball = new EntityBallistaBolt(worldObj, this,
				isSuperPowered());
	...
	{
		worldObj.playSoundAtEntity(this, "random.explode", 8.0F,
				1.0F / (rand.nextFloat() * 0.8F + 0.9F));
		worldObj.playSoundAtEntity(this, "ambient.weather.thunder", 8.0F,
		...
		for (int i = 0; i < 20; i++)
		{
			worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL,
					posX + d + rand.nextDouble() * 0.5D - 0.25D, posY + rand.nextDouble() * 0.5D,
					posZ + d1 + rand.nextDouble() * 0.5D - 0.25D, rand.nextDouble() * 0.1D - 0.05D,
					rand.nextDouble() * 0.1D - 0.05D, rand.nextDouble() * 0.1D - 0.05D);
		}
	...
	public void setCurrentDamage(float i)
	{
		dataWatcher.updateObject(19, Float.valueOf(i));
	}
