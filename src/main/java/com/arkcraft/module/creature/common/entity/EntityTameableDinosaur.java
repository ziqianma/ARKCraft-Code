...
public abstract class EntityTameableDinosaur extends EntityTameable
{
	...
	protected EntityTameableDinosaur(World worldIn, SaddleType saddleType)
	{
		...
		this.tamingSeconds = 120; // Set this before the InventoryTaming (this
	...
	protected EntityTameableDinosaur(World worldIn, SaddleType saddleType, boolean isTameable, int tamingSeconds)
	{
		...
		if (isTameable)
		{
			this.tamingSeconds = tamingSeconds; // This must be before the
												// InventoryTaming
			this.invTamedDino = new InventoryDino("Items", true, saddleType.getInventorySize());
			this.invTaming = new InventoryTaming(this);
			this.invSaddle = new InventorySaddle();
	...
	/**
	 * Clears previous AI Tasks, so the ones defined above will actually
	 * perform.
	 */
	...
	public void increaseTorpor(int i)
	{
		if (this.isTameable())
		{
			...
			LogHelper.info("EntityTameableDinosaur: Set torpor to " + this.invTaming
					.getTorporTime());
		}
	}
	...
	public boolean attackEntityAsMob(Entity entity)
	{
		...
		double attackDamage = this.getEntityAttribute(SharedMonsterAttributes.attackDamage)
				.getBaseValue();
	...
	public boolean interact(EntityPlayer player)
	{
		...
		if (isTamed())
		{
			if (this.isOwner(player))
			{
				...
				else if (itemstack != null)
				{
					...
					if (itemstack != null && isFavoriteFood(itemstack))
					{
						...
						{
							player.addChatMessage(new ChatComponentText(
									"This dino's health is: " + this.getHealth() + " Max is: " + this
			...
			{ // end of owner's dino
				player.addChatMessage(new ChatComponentText(
						"EntityTameableDinosaur: This dino is tamed, but not yours."));
			}
		} // end of is tamed
		...
		else if (isTaming())
		{
			if (!this.worldObj.isRemote)
			{
				...
				LogHelper
						.info("EntityTameableDinosaur: Opening GUI on Dino at: " + this.posX + "," + this.posY + "," + this.posZ + " (" + (int) Math
								.floor(this.posX) + "," + (int) this.posY + "," + (int) Math
								.floor(this.posZ) + ")");
				return true;
			}
			// player.addChatMessage(new
			// ChatComponentText("EntityTameableDinosaur: Use a Raw Porkchop to tame the dino."));
		}
		return false;
	...
	public boolean isUseableByPlayer(EntityPlayer player)
	{
		...
		return player.getDistanceSq(this.posX + X_CENTRE_OFFSET, this.posY + Y_CENTRE_OFFSET,
				this.posZ + Z_CENTRE_OFFSET) < MAXIMUM_DISTANCE_SQ;
	}
	 ...
	 * this one. Override this as needed.
	 */
	@Override
	...
	/**
	 * Dead and sleeping entities cannot move This keeps dino from moving on its
	 * own when you ride it (I think)
	 */
	 ...
	 * This is needed for rider to move dino! TODO: Add ability to jump if
	 * desired
	 */
	public void moveEntityWithHeading(float strafe, float forward)
	{
		if (this.riddenByEntity != null && this.riddenByEntity instanceof EntityPlayer && this
				.isSaddled())
		{
			...
			// if (forward > 0.0F)
			// {
			// float f2 = MathHelper.sin(this.rotationYaw * (float)Math.PI /
			// 180.0F);
			// float f3 = MathHelper.cos(this.rotationYaw * (float)Math.PI /
			// 180.0F);
