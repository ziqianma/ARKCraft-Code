...
public abstract class RangedComponent extends AbstractWeaponComponent
{
	...
	public void addItemAttributeModifiers(Multimap<String, AttributeModifier> multimap)
	{
		multimap.put(WeaponModAttributes.RELOAD_TIME.getAttributeUnlocalizedName(),
				new AttributeModifier("Weapon reloadtime modifier", rangedSpecs.getReloadTime(), 0));
	}
			...
			else
			{
				...
				// i.e. "20 ammo"
				{
					entityplayer.addChatMessage(new ChatComponentText(
							getAmmoQuantity(entityplayer) + StatCollector
	...
	{
		return entityplayer.capabilities.isCreativeMode || EnchantmentHelper.getEnchantmentLevel(
				Enchantment.infinity.effectId, itemstack) > 0 || entityplayer.inventory
				.consumeInventoryItem(getAmmoItem());
	}
	...
	public static enum RangedSpecs
	{
		...
		public int getReloadTime()
		{
			if (reloadTime < 0 && ARKCraft.instance != null)
			{
				...
				ARKCraft.modLog
						.debug("Found reload time " + reloadTime + " for " + reloadTimeId + " @" + this);
				System.out
						.println("Found reload time " + reloadTime + " for " + reloadTimeId + " @" + this);
			}
			return reloadTime;
		}
		...
		public Item getAmmoItem()
		{
			if (ammoItem == null && ammoItemTag != null)
			{
				...
				ARKCraft.modLog
						.debug("Found item " + ammoItem + " for " + ammoItemTag + " @" + this);
	...
	public boolean canUseProjectile(ItemProjectile item)
	{
		return this.projectiles.contains(item.getUnlocalizedName());
	}

	public boolean registerProjectile(ItemProjectile item)
	{
		return this.projectiles.add(item.getUnlocalizedName());
	}
}
