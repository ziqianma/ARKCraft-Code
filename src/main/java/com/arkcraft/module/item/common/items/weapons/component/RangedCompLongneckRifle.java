...
public class RangedCompLongneckRifle extends RangedComponent
{
	...
	public void effectReloadDone(ItemStack itemstack, World world, EntityPlayer entityplayer)
	{
		...
		// world.playSoundAtEntity(entityplayer, "random.door_close", 1.2F, 1.0F
		// / (weapon.getItemRand().nextFloat() * 0.2F + 0.0F));
		world.playSoundAtEntity(entityplayer, ARKCraft.MODID + ":" + "longneck_rifle_reload", 0.7F,
				0.9F / (weapon.getItemRand().nextFloat() * 0.2F + 0.0F));
	}
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer)
	{
		...
		if (!world.isRemote && KeyBindings.attachment.isKeyDown())
		{
			...
			{
				entityplayer.openGui(ARKCraft.instance, GlobalAdditions.GUI.ATTACHMENT_GUI.getID(),
						world, 0, 0, 0);
			}
	...
	/*
	 * @SuppressWarnings("unused") private void setLastUseTime(ItemStack stack,
	 * long time) { stack.setTagInfo("LastUse", new NBTTagLong(time)); }
	 * 
	 * private long getLastUseTime(ItemStack stack) { return
	 * stack.hasTagCompound() ? stack.getTagCompound().getLong( "LastUse") : 0;
	 * }
	 */
	...
	public void effectShoot(World world, double x, double y, double z, float yaw, float pitch)
	{
		...
		world.playSoundEffect(x, y, z, ARKCraft.MODID + ":" + "longneck_rifle_shoot", 1.5F,
				1F / (weapon.getItemRand().nextFloat() * 0.4F + 0.7F));
	...
	/*
	 * @Override public boolean ifCanScope() { return true; }
	 */
}