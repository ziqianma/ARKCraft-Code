...
public class EntityBallista extends EntityBoat
{
    ...
    public boolean attackEntityFrom(DamageSource damagesource, float damage)
    {
        ...
        {
            if (((EntityDamageSource) damagesource).getEntity() == riddenByEntity)
            {
                return true;
            }
        }
        else if (damagesource instanceof EntityDamageSource)
        {
            if (((EntityDamageSource) damagesource).damageType.equals("player"))
            {
                ...
                {
                    dropItem(ARKCraftItems.ballista, 1);
                    if (isLoaded() || isLoading())
                    {
                        dropItem(ARKCraftItems.ballista_bolt, 1);
...
//		if (!worldObj.isRemote)
        //	{
        EntityBallistaBolt entitycannonball = new EntityBallistaBolt(worldObj, this, isSuperPowered());
    ...
    public boolean interactFirst(EntityPlayer entityplayer)
    {
        ...
        if (item == ARKCraftItems.ballista_bolt && !isLoaded() && !isLoading())
        {
            if (entityplayer.inventory.consumeInventoryItem(Items.gunpowder))
            {
                if (entityplayer.inventory.consumeInventoryItem(ARKCraftItems.ballista_bolt))
                {
                    startLoadingCannon();
                    return false;
                }
