            ...
            if (playerIn.canPlayerEdit(pos, side, stack) && playerIn.canPlayerEdit(blockpos1, side, stack)) {
                if (flag2 && flag3 && World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && World.doesBlockHaveSolidTopSurface(worldIn, blockpos1.down())){
                    IBlockState iblockstate1 = ARKCraftBlocks.smithy.getDefaultState()
                    		.withProperty(BlockInventorySmithy.FACING, enumfacing1)
                    		.withProperty(BlockInventorySmithy.PART, BlockInventorySmithy.EnumPartType.RIGHT);
                    if (worldIn.setBlockState(pos, iblockstate1, 3)){
                        IBlockState iblockstate2 = iblockstate1
                        		.withProperty(BlockInventorySmithy.PART, BlockInventorySmithy.EnumPartType.LEFT);
