...
public class BlockInventoryCompostBin extends BlockContainer{
    ...
    public static final PropertyEnum PART = PropertyEnum.create("part", BlockInventoryCompostBin.EnumPartType.class);
...
//		this.setCreativeTab(GlobalAdditions.tabARK); // There is now an item for this
	...
	public boolean onBlockActivated(World worldIn, BlockPos blockPos, IBlockState state, EntityPlayer playerIn, EnumFacing side,	
        ...
        if (state.getValue(PART) != BlockInventoryCompostBin.EnumPartType.LEFT){
//        	blockPos = blockPos.offset((EnumFacing)state.getValue(FACING));  // Original, from bed
...
//        else if (worldIn.getBlockState(pos.offset(enumfacing)).getBlock() != this) { // Original, from bed
    ...
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return state.getValue(PART) == BlockInventoryCompostBin.EnumPartType.LEFT ? null : ARKCraftItems.item_compost_bin;
    }
    ...
    public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
        if (state.getValue(PART) == BlockInventoryCompostBin.EnumPartType.RIGHT) {
    ...
    public IBlockState getStateFromMeta(int meta) {
        ...
        return (meta & 8) > 0 ? this.getDefaultState().withProperty(PART, BlockInventoryCompostBin.EnumPartType.LEFT).withProperty(FACING, enumfacing) 
        		: this.getDefaultState().withProperty(PART, BlockInventoryCompostBin.EnumPartType.RIGHT).withProperty(FACING, enumfacing);
    }
