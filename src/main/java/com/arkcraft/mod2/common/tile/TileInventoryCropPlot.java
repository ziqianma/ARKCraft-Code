...
import com.arkcraft.mod2.common.blocks.BlockCropPlot;
import com.arkcraft.mod2.common.config.MOD2_BALANCE;
import com.arkcraft.mod2.common.items.ItemARKSeed;
import com.arkcraft.mod2.common.items.ItemFeces;
...
public class TileInventoryCropPlot extends TileEntity implements IInventory, IUpdatePlayerListBox {
	...
	private short waterTimeInitialValue = (short) MOD2_BALANCE.CROP_PLOT.SECONDS_OF_WATER_PER_BUCKET; // vanilla value is 1080 = 18 minutes
	...
	private static final short GROW_TIME_FOR_BERRY = (short) MOD2_BALANCE.CROP_PLOT.GROW_TIME_FOR_BERRY;  // vanilla value is 45 seconds
...
//				LogHelper.info("TileInventoryCropPlot: About to harvest a berry on " + (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT ? "client" : "server"));
	...
	private void setGrowthStage(boolean grow) {
		...
		if (grow) {
			...
			// seeded
			case 1:
				if (growTime > MOD2_BALANCE.CROP_PLOT.SECONDS_UNTIL_FRUITLING / 4) {
					growthStage++;
				}
				break;
			// seedling
			case 2:
				if (growTime > MOD2_BALANCE.CROP_PLOT.SECONDS_UNTIL_FRUITLING / 3) {
					growthStage++;
				}
				break;
			// midling
			case 3:
				if (growTime > MOD2_BALANCE.CROP_PLOT.SECONDS_UNTIL_FRUITLING / 2) {
					growthStage++;
				}
				break;
			// growthling
			case 4:
				if (growTime > MOD2_BALANCE.CROP_PLOT.SECONDS_UNTIL_FRUITLING) {
					growthStage++;
					growTime = 1;
				}
				break;
